...
const minifyStaticCss = () =>
	gulp
		...
		.pipe(gulp.dest(getThemeDir()))

const moveScripts = () =>
	gulp.src(PATHS.js.libs).pipe(gulp.dest(getThemeDir('dist/js')))
...
const compressSVGs = () =>
	gulp
		...
		.pipe(gulp.dest(getThemeDir('dist')))
		.pipe(browserSync.stream())

const images = () =>
	gulp
		.src(getStaticPaths(PATHS.images))
		.pipe(changed(getThemeDir('dist')))
		.pipe(
			imagemin([
